...
X = load_boston() # 波士顿房价的数据
print(X['DESCR'])
print(X['data'])
X,Y = load_boston(return_X_y=True)#使数据分开返回 y：房价


# 2. pandas 读取磁盘文件
import  pandas as pd  #基于 xlrd 库，（先装xlrd 后装 pandas）
...
DATA = pd.read_csv(
    filepath_or_buffer=DATA_path,encoding='gbk'
)
#或直接 data = pd.read_csv(path)
...
"""
1.缺失值处理
https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.fillna.html?highlight=pandas%20dataframe%20fillna#pandas.DataFrame.fillna
插值，填充，删除
...
3.降维 or 特征提取
    比如有数据多余，需要数据降维（多维没办法画图，降成两维）
PCA
https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.PCA.html?highlight=pca#sklearn.decomposition.PCA.fit
...
#3sigm原则删除
mu_plus_3sigma = data.loc['mean']+3data.loc['std']


#图
import  matplotlib.pyplot as plt
#箱线图
plt.boxplot(data['平均成绩'])
...
pca = PCA(n_components=2)#定义模型，降成2维
pca.fit(X)
#pca.fit()
#data.iloc[:,    :-1]#索引位置，行，列
PCA(n_components=2)
